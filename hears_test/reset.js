...
module.exports = (controller) => {
  ...
  controller.hears(new RegExp(/^(reset)$/), ['message', 'direct_message'], async (bot, message) => {
    const { text } = message;
    ...
    try {
      ...
      recentUsers = []; // reset recent_users list
      ...
      await bot.say(`[${text}] Recent users list is empty (${result}).`);
    } catch (error) {
      console.log(error);
    }
  });
