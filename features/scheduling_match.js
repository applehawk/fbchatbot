...
module.exports = async (controller) => {
  ...
  //  */
  // const sessionStart = new Date(days <= 1 ? Date.now() : new Date().setHours((7 - days + 1) * 24 + 12, 0, 0));
  ...
  const job = new CronJob(
    ...
    async () => {
      ...
      console.log('[scheduling_match.js:65]: users', count);
      ...
      if (count) {
        ...
        usersList.forEach(async ({ id, state }, i) => {
          ...
          const task = setTimeout(async () => {
            ...
            const senderIndex = Object.keys(users).indexOf(
              `facebook/users/${id}/`
            );
